...
from some_ideas import lu_iteration,filterByDate,fast_search,accurate_search
    ...
    for item in iterator:
        ...
        if k not in base:
            ...
            if not_sub:
                base[k].append(s)
                yield item
chains = srcs_rdd.map(lambda x:((x[1][0][0],x[0],x[1][1][0][1],x[1][1][-1][1]),(x[1][0]+[x[0]],x[1][1]))).repartitionAndSortWithinPartitions(P,lambda x:portable_hash((x[0],x[1]))).mapPartitions(main).distinct().repartitionAndSortWithinPartitions(P,lambda x:portable_hash((x[0],x[1]))).mapPartitions(drop_duplicates)
result = chains.zipWithIndex()
def flatID(iterator):
